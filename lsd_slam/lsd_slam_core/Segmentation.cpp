...
Eigen::Vector3f _axis(0,-std::sqrt(2)/2,-std::sqrt(2)/2);
...
bool Segmentation::normalDifferenceError(pcl::ModelCoefficients::Ptr &normal)
{
...
void Segmentation::computeTransformation()
{
    ...
    double theta = theta=std::acos(normalGround.z/sqrt((normalGround.x* normalGround.x)+(normalGround.y*normalGround.y)+(normalGround.z*normalGround.z)));
